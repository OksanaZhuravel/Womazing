...
const PopupForm: React.FC<PopupFormProps> = ({
  ...
  const isFormSubmited = useSelector(
    (state: RootState) => state.form.isFormSubmited,
  )
  ...
  const addForm = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const validationRules: Array<{
      ...
      },
      {
        ...
        errorMessage: "Только + и цифры для номера телефона",
      },
    ]
    ...
    <>
      <form className="popup__form form">
        ...
        {showButton && (
          <Button
            onClick={addForm}
            className={`button ${isFormSubmited ? "" : "disabled"}`}
          >
